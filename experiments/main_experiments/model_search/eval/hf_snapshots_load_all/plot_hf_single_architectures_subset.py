...
def plot_end_to_end_times(data_root_dirs, file_template, models, approaches, data_items, measure_type,
                          plot_save_path, plot_width, set_key,fallback_path, backup_file_template):
    ...
    ax.set_xticklabels([MODEL_NAME_MAPPING[model.replace("-", "/", 1)] for model in models], rotation=15,
    ...
    plt.savefig(os.path.join(plot_save_path, f'{set_key}-{data_items}-{plot_file_name}.svg'))
    plt.savefig(os.path.join(plot_save_path, f'{set_key}-{data_items}-{plot_file_name}.png'))
...
if __name__ == '__main__':
    ...
    model_sets = {
        "subset-long": (
            [GOOGLE_VIT_BASE_PATCH16_224_IN21K, FACEBOOK_DETR_RESNET_50], 4),
        "subset-short": (
            [FACEBOOK_DETR_RESNET_101, FACEBOOK_DINOV2_LARGE, MICROSOFT_RESNET_152], 6),
    }
    for set_key, (models, plot_width) in model_sets.items():
        models = [model_name.replace("/", "-") for model_name in models]
        ...
        for data_items in [2000, 8000]:
            ...
            plot_save_path = os.path.abspath(f'./plots/single_models/{data_items}/{set_key}')
