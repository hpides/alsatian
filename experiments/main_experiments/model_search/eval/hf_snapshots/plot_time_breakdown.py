...
def stacked_bar_plot_three_configurations(config_1, config_2, config_3, file_path, file_name, approaches):
    ...
    # Data preparation
    baseline_values = [config_1.get(category, 0) / 60 for category in categories]
    shift_values = [config_2.get(category, 0) / 60 for category in categories]
    mosix_values = [config_3.get(category, 0) / 60 for category in categories]
...
def plot_approaches_across_memory_config(root_dir, model, items, device, output_path, approaches):
    collected_data = []
    for approach in approaches:
        ...
        file_id = f'des-gpu-imagenette-huggingface-all-hf-architecture-search#approach#{approach}#cache#{device}#snapshot#{model}#models#-1#items#{items}#level#STEPS_DETAILS'
    ...
    plot_file_name = f'time_breakdown-{model}-{items}'
...
if __name__ == '__main__':
    ...
    for items in [2000, 8000]:
        plot_approaches_across_memory_config(root_dir, model, items, device, output_path, approaches)
