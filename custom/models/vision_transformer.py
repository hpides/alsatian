...
class AddPosEmbedding(nn.Module):

    def __init__(self, pos_embedding, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.pos_embedding = pos_embedding

    def forward(self, input1: torch.Tensor):
        x = input1 + self.pos_embedding
        return x


class ClassifierTokenLayer(nn.Module):
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.add_pos_emb_layer = AddPosEmbedding(self.pos_embedding)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, input: torch.Tensor):
        torch._assert(input.dim() == 3, f"Expected (batch_size, seq_length, hidden_dim) got {input.shape}")
        x = self.add_pos_emb_layer(input)
        x = self.dropout(x)
        x = self.layers(x)
        x = self.ln(x)
        return x
...
class VTProcessInput(nn.Module):
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.vt_input_layer = VTProcessInput(image_size, patch_size, hidden_dim, conv_stem_configs)
        ...
        self.cls_token_layer = ClassifierTokenLayer()
        self.seq_length = seq_length

        self.heads = self._heads(hidden_dim, num_classes, representation_size)
    ...
    def _heads(self, hidden_dim, num_classes, representation_size):
...
def _vision_transformer(
    ...
    if weights:
        # match names form PyTorch state dict
        old_names = ["class_token", "conv_proj.weight", "conv_proj.bias"]
        new_names = ["vt_input_layer.class_token", "vt_input_layer.conv_proj.weight", "vt_input_layer.conv_proj.bias"]
        new_weights = ['encoder.add_pos_emb_layer.pos_embedding']
        state_dict = weights.get_state_dict(progress=progress)
        for old, new in zip(old_names, new_names):
            if old in state_dict.keys():
                state_dict[new] = state_dict[old]
                del state_dict[old]
        for new in new_weights:
            state_dict[new] = model.state_dict()[new]
